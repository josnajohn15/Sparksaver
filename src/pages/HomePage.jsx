...
const HomePage = () => {
  ...
  // Load saved devices on first render
  useEffect(() => {
  ...
  const handleAddDevice = () => {
    if (!appliance || !brand || !hours) return alert('⚠️ Please fill all fields');
      ...
      <div className="card">
        ...
        {savedDevices.length === 0 ? (
          ...
          savedDevices.map((d, i) => (
            <div key={d.id} style={{ marginBottom: '0.5rem', background: '#333', padding: '0.5rem', borderRadius: '6px' }}>
              <span>{d.appliance} ({d.brand}) - {d.hours}h/day</span>
              <button onClick={() => handleDelete(d.id)} style={{ float: 'right', background: '#ff4d4d', border: 'none', color: '#fff', borderRadius: '4px', padding: '2px 8px', cursor: 'pointer' }}>🗑️</button>
            </div>
          ))
        )}
